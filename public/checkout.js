...
async function startRazorpayPayment(cartItems, customerData) {
  ...
  return new Promise((resolve, reject) => {
    const options = {
      key: razorpayKey, amount: data.order.amount, currency: 'INR',
      name: 'Moringai', description: 'Murungai Powder Order', order_id: data.order.id,
      handler: async function(response) {
        const verify = await fetch('/api/payments/verify', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(response) });
        const verified = await verify.json();
        if (!verified.success) return reject(new Error('Payment verification failed'));
        const orderRes = await fetch('/api/orders', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({...customerData, items:cartItems, payment:{method:'razorpay',...response}}) });
