...
async function sendWhatsApp(message) {
  ...
  try {
    const res = await fetch(`https://graph.facebook.com/v18.0/${WA_PHONE_ID}/messages`, {
      ...
      body: JSON.stringify({ messaging_product: 'whatsapp', to: WA_NOTIFY_NUMBER, type: 'text', text: { body: message } })
    });
...
app.post('/api/orders', async (req, res) => {
  ...
  const order = {
    ...
    customer, items, total, shipping, paymentMethod,
...
app.listen(PORT, () => console.log(`🌿 Moringai server on port ${PORT}`));
